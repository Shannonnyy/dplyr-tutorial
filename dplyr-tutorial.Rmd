...
# A local dataframe is essentially the same as a dataframe, but it has a nicer printing format 
# it summarizes the number of rows and cols 
...
## filter: Keep ROW matching criteria
...
##select the rows where month and day corresponds to Jan 01 
##to select a certain row, use the [row,col] function
...
##first write the name of the dataframe, condition and other condition
# , is interpretted as and 
## Nothing is modified in the dataframe, it only presents u the data u requested 
## so you can save it into a separate dataframe if you need to use it 
...
```{r}
# dplyr approach
#write the name of data frame, follow by the cols you want to select 
select(flights, DepTime, ArrTime, FlightNum)

# use colon to select multiple consecutive columns, and use `contains` to select column that has a specific keyword
...
# I want to know which carriers have flight delay > 60 
#typically to do this, we need to first select the cols of required data (carrier and delay)
#then we filter thru these rows to get those that have dep delay values > 60 
filter(select(flights, UniqueCarrier, DepDelay), DepDelay > 60)
```

```{r}
# chaining method
flights %>% #datafame then
    select(UniqueCarrier, DepDelay) %>% #operation 1 
    filter(DepDelay > 60) #operation 2
```
...
flights %>%
    ...
    arrange(DepDelay) #ascending sorting by default 
```
...
flights %>%
    ...
    mutate(Speed = Distance/AirTime*60) #define new variable names, need to use $sign when refering to old variable names since we are refering to variable not the col 
#this does not store the new variable. it just prints it and show u

# store the new variable back to the data frame 
#Assign changes back to the dataframe 
flights <- flights %>% mutate(Speed = Distance/AirTime*60)
```


## summarise: Reduce variables to values
### To group data by a certain variable, and aggregate it with an operation 
...
#usually if what we want to do includes "for each..." we likely need to perform group_by
flights %>%
    group_by(Dest) %>% #it arranges by similar dest 
    summarise(avg_delay = mean(ArrDelay, na.rm=TRUE)) #creates a new variable  there are NA values so need to do na.rm = TRUE 
```
